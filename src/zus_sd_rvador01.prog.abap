*&---------------------------------------------------------------------*
*& Report ZUS_SD_RVADOR01
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zus_sd_rvador01 LINE-COUNT 100 MESSAGE-ID vn.
...
*---------------------------------------------------------------------*
  ...
  fcode_beleg_verfuegbarkeit_sw  LIKE t185f-fcode VALUE 'BVFS',
  fcode_gruppen_verfuegbarkeit   LIKE t185f-fcode VALUE 'GVFP',
  ...
  fcode_kondition_anlegen_neu    LIKE t185f-fcode VALUE 'V69A_KOAN',
  ...
  fcode_template_abwicklung      LIKE t185f-fcode VALUE 'DGMD',
  ...
  fcode_stoffberichte            LIKE t185f-fcode VALUE 'IEHS',
  ...
  fcode_cust_header              LIKE t185f-fcode VALUE 'K_CUST_H',
  fcode_cust_item                LIKE t185f-fcode VALUE 'P_CUST_I',
  fcode_cust_over                LIKE t185f-fcode VALUE 'K_CUST_O',
  fcode_msr_item_overview        LIKE t185f-fcode VALUE 'UMSR',
  fcode_msr_item_detail          LIKE t185f-fcode VALUE 'PMSR',
  fcode_msr_next_plants          LIKE t185f-fcode VALUE 'MSR_NEXTP',
  fcode_msr_vendor_adrs          LIKE t185f-fcode VALUE 'MSR_VADR',
  fcode_msr_insp_text            LIKE t185f-fcode VALUE 'MSR_INSTX',
  fcode_msr_fastchg_item         LIKE t185f-fcode VALUE 'SMSR_ITEM',
  fcode_msr_fastchg_approv       LIKE t185f-fcode VALUE 'SMSR_APPR',
  fcode_msr_tracking_item        LIKE t185f-fcode VALUE 'MSR_TRCI',
  fcode_msr_tracking_all         LIKE t185f-fcode VALUE 'MSR_TRCH',
  fcode_msr_next_vbeln           LIKE t185f-fcode VALUE 'MSR_NEXTD',
  fcode_msr_item_default         LIKE t185f-fcode VALUE 'MSR_DEF',
  fcode_msr_customer_rma         LIKE t185f-fcode VALUE 'MSR_CRMA',
  fcode_new_item                 LIKE t185f-fcode VALUE 'ITNE',
...
CONSTANTS: vbtyp_vert(30)           VALUE 'ABCDEFGHIJKLMNOPSTUWX012345678',
           yp_vert(40)              VALUE 'ABCDEFGHIJKLMNOPSTUWX012345678rstuwx',
           " Vertriebsbelege
...
* Verkauf, der aktuelle Beleg ist nicht lieferrelevant
           ...
           btyp_liavre(2)           VALUE '7T',               " Retoure & Anliefer
...
* Bedarfstypen für Verfügbarkeitsprüfung und Bedarfsverarbeitung
           vbtyp_verk_bed(13)       VALUE 'ABCDEFGHIKLW0', " Verkaufsbelege
           vbtyp_auft_bed(6)        VALUE 'CEFHIW',        " Auftragartige
           vbtyp_liun_bed(1)        VALUE 'c',             " ungeprüfte Lieferung
...
* Faktura
           ...
           typ_fakt_basedat(6)      VALUE 'MNOPNS',     " Fakturabeleg relevant für Basisdatum
...
* IS-Oil reserved VBTYP(es) werden nur im Oilsystem verwendet  --------
           ...
           vbtyp_td_loadn           VALUE 's',     "IS-Oil Ladebestätigung,Umbuchung
           vbtyp_td_gainlo          VALUE 't',     "IS-Oil Gewinn / Verlust
           vbtyp_td_return          VALUE 'u',     "IS-Oil Wiedereinlagerung
           vbtyp_td_gr              VALUE 'x',     "IS-Oil Ladebestätigung,WE
...
*     ACHTUNG: FKREL im RMCSSU08 mitpflegen !!!!
  con_fkrel_liefer(11)             VALUE 'AHJKMPQRTUV',
  con_fkrel_liefermenge(12)        VALUE 'AGHJKMPQRTUV',
...
*     Montage, Fertigungsaufträge, Planaufträge, Netzpläne:
  ...
  activity_assembly_co05(4)        VALUE 'CO05',            "N_1084189
  activity_assembly_co41(4)        VALUE 'CO41',            "N_1084189
  activity_assembly_cohv(4)        VALUE 'COHV',            "N_1084189
  activity_assembly_pk12(4)        VALUE 'PK12',            "n_1255510
  activity_assembly_pk13(4)        VALUE 'PK13',            "n_1255510
  activity_assembly_pk21(4)        VALUE 'PK21',            "n_1255510
  activity_assembly_pk22(4)        VALUE 'PK22',            "n_1255510
  activity_assembly_pkrf(4)        VALUE 'PKRF',
...
*     Kopier BAPI
  activity_bapi_copy(4)            VALUE 'BACO',
*     Global Trade
  activity_global_trade(4)         VALUE 'GTM1',
*     Local ATP processing from CRM
  gc_activity_latp(4)              VALUE 'LATP',
*     Lean Order processing (LORD)
  gc_activity_lord(4)              VALUE 'LORD',
...
CONSTANTS: kopgr_mais(4)           TYPE c VALUE 'AM ',
           ...
           kopgr_mit_zmeng(16)     TYPE c VALUE 'GA  KM  LP  VT  '.
...
*-----------------------------------------------------------------------
CONSTANTS :
  cv_sppaym01 TYPE sppaym VALUE '01'. " Payment Service Provider
...
* Text Objects
CONSTANTS: tdobject_vbbk LIKE stxh-tdobject VALUE 'VBBK',
           tdobject_vbbp LIKE stxh-tdobject VALUE 'VBBP'.
* Message Handler
*INCLUDE /smba0/message.
*inc aufgelöst
*----------------------------------------------------------------------*
*   INCLUDE /smba0/MESSAGE                                            *
*----------------------------------------------------------------------*
...
*-----------------------------------------------------------------------
*       ON COMMIT LEVELs                                      v_n_946479
*-----------------------------------------------------------------------
CONSTANTS: level_msg_post TYPE i VALUE 10,
           level_msg_init TYPE i VALUE 20. "post before init  ^_n_946479
*-----------------------------------------------------------------------
*       Branchenschalter
*-----------------------------------------------------------------------
...
*    return_code = 1.
*  ELSE.
**   SCR -> OTF Conversion as of EHP4 for WDA scenario
...
FORM processing USING    iv_screen  TYPE c
  ...
  DATA: ls_head         TYPE vbdka,
        lt_subitem      TYPE vbdpau_t,
        lt_add          TYPE print_t_add_data,
...
*   error handling
  ...
  IF sy-ucomm = 'PRNT'.
    ...
    DATA lv_smtp_addr TYPE TABLE OF ty_email.
...
*       output_options       = ls_composer_param
 ...
 ls_output_info-otfdata = job_output_info-otfdata.
...
* CREATE MESSAGE BODY TITLE AND DESCRIPTION
    i_objtxt = 'Greetings from DelVal! '.
    APPEND i_objtxt.
    i_objtxt = ' '.
    APPEND i_objtxt.

    FIELD-SYMBOLS:<fs> TYPE any .
    ASSIGN ('(SAPMV45A)VBAK-VBELN ') TO <fs>.
    SELECT SINGLE bstkd FROM vbkd INTO @DATA(wa_bstkd) WHERE vbeln = @<fs>.

*   BELOW LOGIC FOR MAIL ATTACHMENT BODY

    DATA(lv_text1) = 'Thank you for the order. Attached is the Sales Order acknowledgment for your P.O. -' && wa_bstkd.
    i_objtxt = lv_text1.
    APPEND i_objtxt.
    DATA(lv_text2) = 'Please review the pricing and part numbers and let us know if you find any discrepancies. The scheduled ship dates are listed next to each item. Please let us know if you have any questions or if any changes need to be made.'.
    i_objtxt = lv_text2.
    APPEND i_objtxt.
    DATA(lv_br) = ' '.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text3) = 'Please note: By accepting this order acknowledgement, you are agreeing to abide by and be bound to the attached DelVal Flow Controls terms and conditions.'.
    i_objtxt = lv_text3.
    APPEND i_objtxt.
    DATA(lv_text4) = 'If no response is received within 24 hours, we will consider this acknowledgement as accepted.'.
    i_objtxt = lv_text4.
    APPEND i_objtxt.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text6) = 'We would like to thank you for choosing DelVal Flow Controls.'.
    i_objtxt = lv_text6.
    APPEND i_objtxt.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text7) = 'Your feedback is extremely essential for us to better understand your expectations and help us to identify areas that need improvement.'.
    i_objtxt = lv_text7.
    APPEND i_objtxt.
    DATA(lv_text8) = 'We request you to fill out a short survey that would take less than a minute of your time.'.
    i_objtxt = lv_text8.
    APPEND i_objtxt.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text9) = 'The survey can be accessed here - https://forms.office.com/r/hLAWVeM88a .'.
    i_objtxt = lv_text9.
    APPEND i_objtxt.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text10) = 'Thank you, in advance, for taking the time to fill in this questionnaire. Your response will be analyzed, and the results shared with every relevant DelVal Flow Controls employee.'.
    i_objtxt = lv_text10.
    APPEND i_objtxt.
    DATA(lv_text11) = 'Top Management and the Quality Team always look forward to your feedback.'.
    i_objtxt = lv_text11.
    APPEND i_objtxt.
    i_objtxt = lv_br.
    APPEND i_objtxt .
    DATA(lv_text12) = 'Thank you for your continued business with DelVal Flow Controls USA, LLC.'.
    i_objtxt = lv_text12.
    APPEND i_objtxt.
...
*************************
    ...
    wa_doc_chng-obj_descr = 'Sales Order'. "subject . "'CHECK LETTER'.
...
* ATTACHMENT (PDF-ATTACHMENT)
    ...
    SELECT SINGLE kunnr FROM vbak INTO @DATA(wa_kunnr) WHERE vbeln = @<fs>
                                                      AND vkorg = 'US00'.

    SELECT SINGLE adrnr
                FROM kna1
                INTO lv_adrnr
                WHERE kunnr = wa_kunnr."custno-low.
    ...
    IF lv_smtp_addr IS NOT INITIAL .
      ...
      DATA lv_sender TYPE so_rec_ext VALUE 'purchaseorders@delvalflow.com'.
*      DATA lv_sender TYPE so_rec_ext VALUE 'ithelpdesk@delvalflow.com'.
      WAIT UP TO 2 SECONDS.
***************COMMENTED BY JYOTI ON 02.05.2025*********************
*      DATA wa_rece TYPE somlreci1.                """cc list
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'amodak@delvalflow.com'.
*      wa_rece-copy = 'X'.
...
*
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'clee@delvalflow.com'.
*      wa_rece-copy = 'X'.
*      APPEND wa_rece  TO i_reclist.
*      CLEAR :wa_rece .
*
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'kjones@delvalflow.com'.
*      wa_rece-copy = 'X'.
*      APPEND wa_rece  TO i_reclist.
*      CLEAR :wa_rece .
*
*
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'aveteto@delvalflow.com'.
*      wa_rece-copy = 'X'.
*      APPEND wa_rece  TO i_reclist.
*      CLEAR :wa_rece .
*
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'gfontenot@delvalflow.com'.
*      wa_rece-copy = 'X'.
*      APPEND wa_rece  TO i_reclist.
*      CLEAR :wa_rece .
*
*      wa_rece-rec_type  = 'U'.
*      wa_rece-receiver = 'jlarose@delvalflow.com'.
*      wa_rece-copy = 'X'.
*      APPEND wa_rece  TO i_reclist.
*      CLEAR :wa_rece .
*
...
***********************ADDED BY JYOTI ON 12.02.2025***********************
     ...
     SELECT * FROM ZUS_SO_CC INTO TABLE @DATA(IT_MAIL).
       IF IT_MAIL IS NOT INITIAL.
         ...
         wa_rece-copy = 'X'.
         APPEND wa_rece TO i_reclist.
         CLEAR: WA_MAIL, wa_rece.
         ENDLOOP.
       ENDIF.
*********************************commented by jyoti on 14.04.2025******************************
*      CALL FUNCTION 'SO_DOCUMENT_SEND_API1'     """function module for mail send
*        EXPORTING
...
****************************added by jyoti on 14.04.2025*****************************************************
 DATA: lv_answer     TYPE c,
      lv_counter    TYPE i,
      lv_vbeln      TYPE vbak-vbeln,  " your document number
      lv_outputtype TYPE nast-KSCHL.

 lv_vbeln = <fs>.         " example: Sales order number
lv_outputtype = nast-kschl.       " or whatever type you use
 DATA: ls_counter1 TYPE zus_so_count.

 " Check if mail already sent before
*SELECT SINGLE counter, CREATED_DATE , CREATED_time, USNAM INTO lv_counter, lv
...
if sy-subrc = 0 AND LS_COUNTER1-COUNTER > 0.
  ...
  ls_counter1-CREATED_DATE '.Would you still like to send again?' INTO data(gv_string) SEPARATED BY space.
 " Mail was already sent before, ask user if they want to repeat
    ...
    UPDATE ZUS_so_COUNT SET counter = lv_counter CREATED_DATE = sy-datum CREATED_time = sy-uzeit usnam = sy-uname
...
*
      ENDIF.
       ...
       DATA: ls_counter TYPE zus_so_count.
...
INSERT zus_so_count FROM ls_counter..
...
*       output_options       = ls_composer_param
    ...
    ls_output_info-otfdata = job_output_info-otfdata.        "NEW
...
*Fehlermeldung bei Druckansicht ausgeben
    ...
    ELSE.
      READ TABLE job_output_info-spoolids INTO l_spoolid INDEX 1.
...
*     pi_addr_key   = ls_addr
  ...
  IF cv_retcode = 0.
    ...
    PERFORM check_repeat_new CHANGING lv_repeat.
...
*&---------------------------------------------------------------------*
*&      Form  CHECK_REPEAT_NEW
*&---------------------------------------------------------------------*
FORM check_repeat_new CHANGING cv_repeat TYPE c.
