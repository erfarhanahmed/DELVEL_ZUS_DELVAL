...
TYPES:BEGIN OF TY_T016T,
      ...
      BEGIN OF TY_LIKP,   "Added By Nilay B. On 29.06.2023
...
DATA : IT_VBPA  TYPE TABLE OF TY_VBPA,
       ...
       IT_VBFA3 TYPE TABLE OF TY_VBFA3,
       WA_VBFA3 TYPE          TY_VBFA3,

       IT_VBFA4 TYPE TABLE OF TY_VBFA4,
       WA_VBFA4 TYPE          TY_VBFA4,
...
*    gst_region_sh TYPE zgst_region-gst_region,  "Ship To State Code
    ...
    VRKME        TYPE VBRP-VRKME,              "Unit
    GV_RATE      TYPE CHAR250, "netpr        TYPE char100,              "Rate
    BLART        TYPE BKPF-BLART,              "FI Document Type
    BELNR        TYPE BKPF-BELNR,              "FI Document No.
    MWSKZ        TYPE PRCD_ELEMENTS-MWSK1,              "Taxcode
    TAX_TXT      TYPE T007S-TEXT1,             "Tax Code Description
...
DATA: LV_TEMP_KWERT TYPE PRCD_ELEMENTS-KWERT.
...
**************************** added by shreya 5-06-2023
    SELECT VBELV
              ...
              FOR ALL ENTRIES IN LT_SALES_INV_ITEM
              WHERE VBELN = LT_SALES_INV_ITEM-VBELN
              AND VBTYP_V = 'C' AND VBTYP_N = 'M'.
**************************** end added by shreya 5-06-2023

    """""""""""""""""""""""""""""""""""""""Added by Pranit 29.02.2024
    SELECT VBELV,
        VBELN,
        VBTYP_V,
        VBTYP_N,
...
**********************************************************************************************************************
      ...
      IF SY-SUBRC IS INITIAL.
        ...
        LS_FINAL-FKSTO = LS_SALES_INV_HDR-FKSTO.
      ...
      """""""""""""""""""""""""""""""
      READ TABLE IT_VBAP1 INTO WA_VBAP1 WITH KEY VBELN = LS_SALES_INV_ITEM-AUBEL POSNR = LS_SALES_INV_ITEM-POSNR.      ""Added by Pranit 09.03.2024
      IF SY-SUBRC = 0.
        LS_FINAL-CUSTDELDATE = WA_VBAP1-CUSTDELDATE.
      ENDIF.

      READ TABLE IT_VBFA3 INTO WA_VBFA3 WITH KEY VBELN = LS_SALES_INV_ITEM-VBELN POSNV = LS_SALES_INV_ITEM-POSNR.       ""Added by Pranit 09.03.2024
      IF SY-SUBRC = 0.
        LS_FINAL-POSNV = WA_VBFA3-POSNV.
      ENDIF.
      """"""""""""""""""""""""""""""""
      ...
      IF SY-SUBRC IS INITIAL.
        ...
        IF LS_FINAL-FKSTO EQ 'X'.            """""ADDED BY PRANIT 27.11.2024
...
**************************** added by shreya 5-06-2023
*        READ TABLE it_vbfa3 INTO wa_vbfa3 WITH KEY vbeln = ls_sales_inv_item-vbeln posnv = ls_sales_inv_item-posnr.   COmmented by Pranit 09.03.2024
*        IF sy-subrc = 0.
*          ls_final-posnv = wa_vbfa3-posnv.
*        ENDIF.

        READ TABLE IT_VBFA4 INTO WA_VBFA4 WITH KEY VBELN = LS_SALES_INV_ITEM-VBELN.

        READ TABLE IT_LIKP INTO WA_LIKP WITH KEY VBELN = WA_VBFA4-VBELV.
        IF SY-SUBRC = 0.
          LS_FINAL-BLDAT = WA_LIKP-BLDAT.
        ENDIF.

*        READ TABLE it_vbap1 INTO wa_vbap1 WITH KEY vbeln = wa_vbfa3-vbelv posnr = wa_vbfa3-posnv.  COmmented by Pranit 09.03.2024
*        IF sy-subrc = 0.
*          ls_final-custdeldate = wa_vbap1-custdeldate.
*        ENDIF.
...
*----------------added by DH 02.05.23
            ...
            ENDLOOP.
*            IF ls_accounting_mdoc_item-shkzg = 'H'.               "commented by DH
*              gv_dmbtr = gv_dmbtr * -1.
*            ENDIF.
          ...
          ENDLOOP.
          CLEAR : GV_DMBTR. "added by shreya 18-10-22
        ENDIF.
...
*          ENDIF.
      ...
      IF SY-SUBRC IS INITIAL.
        ...
        WA_FINAL-ZBSTKD = LS_SALES_ORD_ITEM-ZBSTKD.
        WA_FINAL-ZPOSEX = LS_SALES_ORD_ITEM-ZPOSEX.
**
        LS_FINAL-ZBSTKD = LS_SALES_ORD_ITEM-ZBSTKD.
        LS_FINAL-ZPOSEX = LS_SALES_ORD_ITEM-ZPOSEX.
      ENDIF.
...
***added by shreya******
      READ TABLE IT_KNA11 INTO WA_KNA11 WITH KEY KUNNR = LS_SALES_PARTNER-KUNNR.
      IF SY-SUBRC IS INITIAL.
        LS_FINAL-STRAS = WA_KNA11-STRAS.
      ENDIF.
...
*      READ TABLE LT_CONDITIONS INTO LS_CONDITIONS WITH KEY KNUMV = LS_SALES_INV_HDR-KNUMV              "COMMENTED BY VINAYAK S.
*                                                           KPOSN = LS_SALES_INV_ITEM-POSNR
*                                                           KSCHL = 'UHF1'
*                                                           KSTAT = SPACE.

      LOOP AT LT_CONDITIONS INTO LS_CONDITIONS WHERE KNUMV = LS_SALES_INV_HDR-KNUMV
                                               AND KPOSN = LS_SALES_INV_ITEM-POSNR
                                               AND KSCHL = 'UHF1'
                                               AND KSTAT = SPACE.
        IF LS_CONDITIONS-KWERT GT 0.
          LV_TEMP_KWERT = LS_CONDITIONS-KWERT + LV_TEMP_KWERT.
        ENDIF.

      ENDLOOP.

      IF SY-SUBRC IS  INITIAL.
        LS_FINAL-PF_VAL = LV_TEMP_KWERT * LS_SALES_INV_HDR-KURRF.
...
*                                                           KSTAT = SPACE.
      ...
      LOOP AT LT_CONDITIONS INTO LS_CONDITIONS WHERE KNUMV = LS_SALES_INV_HDR-KNUMV       "Added by Vinayak S.
                                               AND KPOSN = LS_SALES_INV_ITEM-POSNR
                                               AND KSCHL = 'UMC1'
                                               AND KSTAT = SPACE.
        IF LS_CONDITIONS-KWERT GT 0.
      ...
      IF SY-SUBRC IS  INITIAL.
        LS_FINAL-FRT = LV_TEMP_KWERT * LS_SALES_INV_HDR-KURRF.
...
* *****     *************added by shreya 15_07_22***************
      ...
      IF LS_SALES_INV_HDR-FKART = 'US11' AND LS_SALES_INV_HDR-FKART_RL = 'LG'.
        ...
        IF LS_ACCOUNTING_MDOC_ITEM-SHKZG NE 'S'.                " H => S      CHanged by Vinayak
      ...
      IF LS_SALES_INV_HDR-FKART = 'US03'.
        LS_FINAL-GV_COGS = LS_FINAL-GV_COGS * -1.
      ENDIF.
*
      READ TABLE IT_VBFA_N INTO DATA(WA_VBFA_N) WITH KEY VBELV = LS_SALES_INV_ITEM-VGBEL.   """ADDED BY PRANIT 29.02.2024
      IF SY-SUBRC IS INITIAL.
        IF LS_SALES_INV_HDR-FKART = 'US03' AND   WA_VBFA_N-VBTYP_V = 'T'.

          LS_FINAL-GV_COGS = LS_FINAL-GV_COGS * -1.

        ELSEIF WA_VBFA_N IS NOT INITIAL .

          LS_FINAL-GV_COGS = LS_FINAL-GV_COGS .

        ENDIF.
      ENDIF.                                                                                 """"ENDED BY PRANIT 29.02.2024
...
* Implement suitable error handling here
      ...
      CONCATENATE LS_FINAL-AUBEL LS_FINAL-POSNR
           INTO LV_NEW.
      " LV_new = LS_FINAL-VBELN.
      "Port Code
      CLEAR: LT_LINES[],LS_LINES.
      CALL FUNCTION 'READ_TEXT'
        EXPORTING
...
* Implement suitable error handling here
      ...
      IF NOT LT_LINES IS INITIAL.
        LOOP AT LT_LINES INTO LS_LINES.
          IF NOT LS_LINES-TDLINE IS INITIAL.
            CONCATENATE LS_FINAL-SHIP_DATE LS_LINES-TDLINE INTO LS_FINAL-SHIP_DATE SEPARATED BY SPACE.
          ENDIF.
        ENDLOOP.
...
*      READ TABLE IT_US INTO WA_US WITH KEY ZVBELN = LS_SALES_INV_ITEM-AUBEL.            """Added by Pranit 21.02.2024
*      WA_FINAL-ZBSTKD = WA_US-ZBSTKD.
*      WA_FINAL-ZPOSEX = WA_US-ZPOSEX.
*
*      LS_FINAL-ZBSTKD = WA_US-ZBSTKD.
*      LS_FINAL-ZPOSEX = WA_US-ZPOSEX.
...
*      wa_final-gst_region_sh = ls_final-gst_region_sh.
      ...
      WA_FINAL-SHIP_DATE         = LS_FINAL-SHIP_DATE. "added by shreya 12-12-2023
...
*
*
      WA_FINAL-STRAS       = LS_FINAL-STRAS.
      WA_FINAL-POSNV       = LS_FINAL-POSNV.


      WA_FINAL-CUSTDELDATE       = LS_FINAL-CUSTDELDATE.
      ...
      IF  LS_FINAL-CUSTDELDATE  IS NOT INITIAL.
        CALL FUNCTION 'CONVERSION_EXIT_IDATE_OUTPUT'
          ...
          IMPORTING
            OUTPUT = WA_FINAL-CUSTDELDATE.
        CONCATENATE WA_FINAL-CUSTDELDATE+0(2) WA_FINAL-CUSTDELDATE+2(3) WA_FINAL-CUSTDELDATE+5(4)
                       INTO WA_FINAL-CUSTDELDATE SEPARATED BY '-'.
      ...
      IF LS_FINAL-FKART = 'F5' OR LS_FINAL-FKART = 'F8' OR LS_FINAL-FKART = 'ZPI' OR LS_FINAL-FKART = 'US09' OR LS_FINAL-FKART = 'US12' OR LS_FINAL-FKART = 'F5'
       OR LS_FINAL-FKART = 'F8' OR LS_FINAL-FKART = 'US13'.
        ""ADD LOGIC FOR REMOVE PERFORMACE ORDER TYPE   ADDED BY PRANIT 20.12.2024
      ELSE.
      ...
      CLEAR:
         ...
         LS_SALES_BUSS,LT_LINES,LS_LINES,LT1_LINES,LS1_LINES,BLANK,DES,LS_SAL_TXT, LS_SAL_TXT,NAME,COUNT,LS_VBFA, WA_VBFA3, WA_VBAP1.
...
*  ls_fieldcat-outputlen = '25'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'Ship To House/ Street'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.
  ...
  GV_POS = GV_POS + 1.
  LS_FIELDCAT-FIELDNAME = 'POSNV'.
*  ls_fieldcat-outputlen = '25'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'SO Item Number'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.
  ...
  GV_POS = GV_POS + 1.
  LS_FIELDCAT-FIELDNAME = 'CUSTDELDATE'.
*  ls_fieldcat-outputlen = '25'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'Cust Del Date'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.
...
*  ls_fieldcat-hotspot   = 'X'.
  ...
  GV_POS = GV_POS + 1.
  LS_FIELDCAT-FIELDNAME = 'SHIP_DATE'.
*  ls_fieldcat-outputlen = '25'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'Shipping From'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.
  ...
  GV_POS = GV_POS + 1.
  LS_FIELDCAT-FIELDNAME = 'ZBSTKD'.              """Added by Pranit 21.02.2024
*  ls_fieldcat-outputlen = '5'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'DV IND PO Number'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.

  GV_POS = GV_POS + 1.                            """Added by Pranit 21.02.2024
  LS_FIELDCAT-FIELDNAME = 'ZPOSEX'.
*  ls_fieldcat-outputlen = '25'.
  LS_FIELDCAT-TABNAME   = 'GT_FINAL'.
  LS_FIELDCAT-SELTEXT_L = 'DV IND PO Line Item Number'.
  LS_FIELDCAT-COL_POS   = GV_POS.
*  ls_fieldcat-hotspot   = 'X'.
  APPEND LS_FIELDCAT TO CT_FIELDCAT.
  CLEAR LS_FIELDCAT.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  IF SY-SUBRC = 0.
    DATA LV_STRING_4758 TYPE STRING.
    DATA LV_CRLF_4758 TYPE STRING.
    LV_CRLF_4758 = CL_ABAP_CHAR_UTILITIES=>CR_LF.
    LV_STRING_4758 = HD_CSV.
    LOOP AT IT_CSV INTO WA_CSV.
      CONCATENATE LV_STRING_4758 LV_CRLF_4758 WA_CSV INTO LV_STRING_4758.
      CLEAR: WA_CSV.
    ENDLOOP.
    TRANSFER LV_STRING_4758 TO LV_FULLFILE.
...
*  lv_folder = 'D:\usr\sap\DEV\D00\work'.
  ...
  OPEN DATASET LV_FULLFILE
     FOR OUTPUT IN TEXT MODE ENCODING DEFAULT.  "NON-UNICODE.
  IF SY-SUBRC = 0.
    DATA LV_STRING_4798 TYPE STRING.
    DATA LV_CRLF_4798 TYPE STRING.
    LV_CRLF_4798 = CL_ABAP_CHAR_UTILITIES=>CR_LF.
    LV_STRING_4798 = HD_CSV.
    LOOP AT IT_CSV INTO WA_CSV.
      CONCATENATE LV_STRING_4798 LV_CRLF_4798 WA_CSV INTO LV_STRING_4798.
      CLEAR: WA_CSV.
    ENDLOOP.
    TRANSFER LV_STRING_4798 TO LV_FULLFILE.
    CONCATENATE 'File' LV_FULLFILE 'downloaded' INTO LV_MSG SEPARATED BY SPACE.
    MESSAGE LV_MSG TYPE 'S'.
...
*              'Std. Cost'
              ...
              'COGS Doc. No'
              'COGS Amount'
              'Ship To House/Street'
              'SO Item Number'
              'Cust Del Date'
              'Delivery Date'
              'Shipping From'
              'DV IND PO Number'
              'DV IND PO Line Item Number'
  INTO PD_CSV
