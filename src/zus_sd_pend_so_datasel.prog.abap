...
*      READ TABLE it_t052u INTO wa_t052u WITH KEY zterm = wa_vbkd-zterm.
      READ TABLE it_tvzbt INTO wa_tvzbt WITH KEY zterm = wa_vbkd-zterm.
...
***      ENDIF.
      ...
      READ TABLE it_partner INTO wa_partner WITH KEY kunnr = wa_vbpa-kunnr.
      IF sy-subrc = 0.
...
*        wa_output-sale_off   = wa_knvv-vkbur.
*        wa_output-bzirk      = wa_knvv-bzirk.
      ENDIF.
...
*          CONCATENATE wa_output-desc wa_lines-tdline INTO wa_output-desc SEPARATED BY space.
*        ENDIF.
*      ENDLOOP.
*      CONDENSE wa_output-desc.
*    ENDIF.
**ECCN
     CLEAR: lv_lines, wa_lines.
      REFRESH lv_lines.
...
*              IF sy-subrc = 0.
*                wa_output-wip = wa_output-wip + wa_afpo-pgmng - wa_caufv-igmng .
*              ENDIF.
*            ENDIF.
*          ENDIF.
...
*  PERFORM build_fieldcat USING 'ZLDPERWEEK'     'X' '17' 'LD %Per Week'(046).
*  PERFORM build_fieldcat USING 'ZLDMAX'        'X' '18'  'LD % Max'(047).
*  PERFORM build_fieldcat USING 'ZLDFROMDATE'    'X' '19' 'LD From Date'(048).
...
*  PERFORM build_fieldcat USING 'DESC'           'X' '29'   'Item Long Text'(018).
  PERFORM build_fieldcat USING 'KWMENG'         'X' '31'   'SO QTY'(019).
*  PERFORM build_fieldcat USING 'KALAB'          'X' '25'   'Stock Qty'(020).
*  PERFORM build_fieldcat USING 'STOCK_QTY'          'X' '24'   'Stock Qty'(020).
...
*  PERFORM build_fieldcat USING 'CURR_CON'       'X' '33'   'Currency Conv'(028).
*  PERFORM build_fieldcat USING 'SO_EXC'       'X' '34'   'SO Exchange Rate'(051).
  PERFORM build_fieldcat USING 'AMONT'          'X' '40'   'Pending SO Amount'.
  PERFORM build_fieldcat USING 'ORDR_AMT'       'X' '41'   'Order Amount'(030).
*  PERFORM BUILD_FIELDCAT USING 'KURSK'          'X' '34'   ''(031).
*  PERFORM build_fieldcat USING 'IN_PRICE'        'X' '37'   'Internal Price'(032).
*  PERFORM build_fieldcat USING 'IN_PR_DT'        'X' '38'   'Internal Price Date'(033).
*  PERFORM build_fieldcat USING 'EST_COST'        'X' '39'   'Estimated Cost'(034).
*  PERFORM build_fieldcat USING 'LATST_COST'      'X' '40'   'Latest Estimated Cost'(035).
  PERFORM build_fieldcat USING 'ST_COST'         'X' '42'   'Standard Cost'(036).
  PERFORM build_fieldcat USING 'ZSERIES'         'X' '43'   'Series'(037).
  PERFORM build_fieldcat USING 'ZSIZE'           'X' '44'   'Size'(038).
  PERFORM build_fieldcat USING 'BRAND'           'X' '45'   'Brand'(039).
  PERFORM build_fieldcat USING 'MOC'             'X' '46'   'MOC'(040).
  PERFORM build_fieldcat USING 'TYPE'            'X' '47'   'Type'(041).
  """"""""""""'   Added By KD on 04.05.2017    """""""
  PERFORM build_fieldcat USING 'DISPO'            'X' '48'   'MRP Controller'(051).
*  PERFORM build_fieldcat USING 'WIP'              'X' '48'   'WIP'(052).
  PERFORM build_fieldcat USING 'MTART'            'X' '49'   'MAT TYPE'.
*  PERFORM build_fieldcat USING 'KDMAT'            'X' '50'   'CUST MAT NO'.

*  PERFORM build_fieldcat USING 'QMQTY'            'X' '52'   'QM QTY'.

*  PERFORM build_fieldcat USING 'ITMTXT'           'X' '54'   'Sales Text'.
...
*  PERFORM build_fieldcat USING 'OFM'              'X' '61'   'OFM No.'.
*  PERFORM build_fieldcat USING 'OFM_DATE'         'X' '62'   'OFM Date'.
*  PERFORM build_fieldcat USING 'SPL'              'X' '49'   'ECCN'.
  PERFORM build_fieldcat USING 'CUSTDELDATE'    'X' '56'  'Customer Delivery Date'.   "(015).
  PERFORM build_fieldcat USING 'BNAME'          'X' '57'  'Contact Person'.   "(015).
  PERFORM build_fieldcat USING 'ECCN'          'X' '58'  'ECCN'.   "(015).
  ...
  PERFORM build_fieldcat USING 'SHIP_NAME'        'X' '60'  'Ship To Party Name'.
  PERFORM build_fieldcat USING 'STRAS'            'X' '60'  'Ship To Party House No'.
  PERFORM build_fieldcat USING 'PSTLZ'            'X' '60'  'Ship To Party Postal Code'.
  PERFORM build_fieldcat USING 'ORT01'            'X' '60'  'Ship To Party City'.
  PERFORM build_fieldcat USING 'SHIP_RIG'         'X' '60'  'Ship To Party Region'.
  PERFORM build_fieldcat USING 'SHIP_LAND'        'X' '60'  'Ship To Party Country'.
  """"""""""""""""""""""""""""""""""""""""""""""""""""
